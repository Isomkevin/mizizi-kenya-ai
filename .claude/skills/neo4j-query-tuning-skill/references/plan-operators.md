...
**Lazy vs Eager**: Most operators stream rows to parent as produced. Eager operators (marked ✗ below) must consume _all_ input before emitting output — they materialise the full row set and can cause OOM on large inputs.
